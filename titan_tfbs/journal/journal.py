...
TRADE_COLUMNS = [
    ...
    "exit_reason",
    "chart_reference",
    # Ch XIII-A asks for the reasoning as well as the levels. The full note
    # goes to the markdown research log; this is its thesis paragraph, so the
    # CSV row stands on its own at review time.
    "research",
    "notes",
]
...
class TradeJournal:
    ...
    def __init__(self, config: JournalConfig, root: Optional[str | Path] = None) -> None:
        ...
        self.research_path = base / config.research_log
        self._trades: Optional[_Writer] = None
        self._signals: Optional[_Writer] = None
        self._equity: Optional[_Writer] = None
        self.records: List[Dict[str, object]] = []
        #: Research notes in write order, for reporting without re-reading the file.
        self.research: List[Dict[str, object]] = []
        if self.enabled:
    ...
    def log_trade(self, position: Position, asset_class: str = "") -> Dict[str, object]:
        ...
        row: Dict[str, object] = {
            ...
            "research": signal.research.thesis if signal.research else "",
    ...
    # -- Ch XIII-A: the reasoning ------------------------------------------

    def log_research(self, note) -> None:
        """Append a research note to the markdown log.

        Ch XIII-A's record set is levels *and* reasoning. The CSVs carry the
        levels; this carries the "why", in the form a human reads at review
        time. Notes are appended in decision order, so the file reads as the
        desk's running commentary on the session.
        """
        if not self.enabled or not self.config.log_research:
            return
        if not note.taken and not self.config.log_research_for_skipped:
            return
        self.research.append(note.to_dict())
        self.research_path.parent.mkdir(parents=True, exist_ok=True)
        new_file = not self.research_path.exists()
        with open(self.research_path, "a") as handle:
            if new_file:
                handle.write(
                    "# TFBS research log\n\n"
                    "Ch XIII-A record of why each setup qualified. Generated by "
                    "the engine from the same objects it decided on.\n\n"
                )
            handle.write(note.to_markdown())
            handle.write("\n")
